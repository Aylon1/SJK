...
"""
BinanceSpotAccount Class Documentation

This class provides a comprehensive interface for interacting with a Binance spot account. 
It supports real-time balance updates, order tracking, and trade monitoring through websockets, 
along with various utility methods for account management and analysis.

Key Features:
- Real-time balance updates via websocket.
- Tracking of open orders and trade history.
- Callback support for balance, order, and trade updates.
- Portfolio analysis tools including PnL, risk metrics, and position exposure.
- Thread-safe operations for concurrent access.

Example Usage:
    >>> from binance_spot_account import BinanceSpotAccount
    >>> account = BinanceSpotAccount(api_key="your_api_key", api_secret="your_api_secret", use_testnet=True)
    >>> account.start()  # Start real-time monitoring

    # Add callbacks for real-time updates
    >>> def balance_callback(asset, free, locked):
    ...     print(f"Balance update: {asset} - Free: {free}, Locked: {locked}")
    >>> account.add_balance_callback(balance_callback)
...
Attributes:
    ...
    use_testnet (bool): Whether to use Binance testnet (default: True).
    tracked_currencies (List[str]): List of currencies to track (e.g., ['BTC', 'ETH']).
    balances (Dict[str, Dict[str, Decimal]]): Dictionary of current balances.
    orders (DefaultDict[str, Dict]): Dictionary of open orders.
    trades (DefaultDict[str, List]): List of trade history.
    order_updates (DefaultDict[str, List]): List of order updates.
    _balance_callbacks (List[Callable]): List of balance update callbacks.
    _order_callbacks (List[Callable]): List of order update callbacks.
    _trade_callbacks (List[Callable]): List of trade update callbacks.
    is_running (bool): Whether the real-time monitoring is active.

Methods:
    __init__(api_key: str, api_secret: str, use_testnet: bool = True, tracked_currencies: Optional[List[str]] = None):
        Initialize the Binance spot account connection.
    ...
    get_balance(symbol: Optional[str] = None) -> Union[Dict, Decimal]:
        Get current balance for all tracked assets or a specific symbol.

    get_formatted_balance(min_value: float = 0.0) -> pd.DataFrame:
        Get formatted balance as a DataFrame, filtered by minimum value.

    get_open_orders(symbol: Optional[str] = None) -> Union[Dict, List[Dict]]:
        Get all open orders or open orders for a specific symbol.

    get_order_history(symbol: Optional[str] = None, status: Optional[str] = None, limit: int = 100) -> pd.DataFrame:
        Get order history with optional filtering.

    get_trade_history(symbol: Optional[str] = None, start_time: Optional[datetime] = None, limit: int = 500) -> pd.DataFrame:
        Get trade history with optional filters.

    get_order_updates(order_id: str) -> List[Dict]:
        Get update history for a specific order.

    add_balance_callback(callback: Callable[[str, Decimal, Decimal], None]) -> None:
        Add callback for balance updates.

    add_order_callback(callback: Callable[[Dict], None]) -> None:
        Add callback for order updates.

    add_trade_callback(callback: Callable[[Dict], None]) -> None:
        Add callback for trade updates.

    get_tracked_symbols() -> List[str]:
        Get list of symbols with non-zero balances or open orders.

    get_daily_pnl(days: int = 30) -> pd.DataFrame:
        Get daily profit/loss analysis.

    get_risk_metrics() -> Dict[str, float]:
        Get current risk metrics.

    format_number_to_binance_precision(symbol: str, number: float, is_price: bool = True) -> str:
        Format numbers according to Binance's precision requirements for a specific symbol.

    get_position_value(symbol: str) -> Dict[str, float]:
        Get the current value of a position in USDT.

    get_position_exposure() -> pd.DataFrame:
        Calculate exposure of all positions relative to total portfolio value.

Callback Examples:
    Balance Callback:
        >>> def balance_callback(asset: str, free: Decimal, locked: Decimal):
        ...     print(f"Balance update: {asset} - Free: {free}, Locked: {locked}")
        >>> account.add_balance_callback(balance_callback)

    Order Callback:
        >>> def order_callback(order: Dict):
        ...     print(f"Order update: {order}")
        >>> account.add_order_callback(order_callback)

    Trade Callback:
        >>> def trade_callback(trade: Dict):
        ...     print(f"Trade update: {trade}")
        >>> account.add_trade_callback(trade_callback)

Thread Safety:
    The class uses threading.Lock to ensure thread-safe operations on shared resources like balances, orders, and trades.

Error Handling:
    The class logs errors using the logging module. Errors during websocket communication or API calls are logged and raised.

Dependencies:
    - binance: Binance API client.
    - pandas: Data manipulation and analysis.
    - asyncio: Asynchronous programming support.
    - nest_asyncio: Allows asyncio to work in Jupyter notebooks.
    - decimal: High-precision decimal arithmetic.
    - threading: Thread safety.
    - collections.defaultdict: Default dictionary for order and trade tracking.
