...
"""
CryptoDataManager: Real-time Cryptocurrency Market Data Handler

A comprehensive solution for managing real-time and historical cryptocurrency market data,
featuring WebSocket connections, order book tracking, and advanced market analytics.
...
# Get market snapshot
snapshot = manager.get_market_snapshot('BTCUSDT')
print(f"Current Price: {snapshot['price']}")
print(f"24h Change: {snapshot['change_24h']}%")

# Monitor connection health
stats = manager.get_connection_statistics()
print(f"Connection Status: {stats['general']['connection_status']}")
print(f"Uptime: {stats['general']['uptime']} seconds")
```
...
strength_data = manager.get_relative_strength('BTC')
print("\nRelative Strength Rankings:")
for symbol, metrics in strength_data['relative_metrics'].items():
    print(f"{symbol}: {metrics['relative_strength']:.4f}")

# Get volume profile analysis
volume_profile = manager.get_volume_profile('BTCUSDT')
print(f"Point of Control: {volume_profile['poc_price']}")
print(f"Value Area: {volume_profile['value_area_low']} - {volume_profile['value_area_high']}")

# Get technical indicators
indicators = manager.get_technical_indicators('BTCUSDT')
...
liquidity = manager.get_liquidity_analysis('BTCUSDT')
print(f"Bid/Ask Ratio: {liquidity['bid_liquidity'] / liquidity['ask_liquidity']}")
print(f"Price Impact (1 BTC): {liquidity['price_impacts'][1.0]}")

# Get order book metrics
book_metrics = manager.get_order_book_metrics('BTCUSDT')
print(f"Spread: {book_metrics['bid_ask_spread']}")
print(f"Depth Imbalance: {book_metrics['imbalance_ratio']}")
```
...
for symbol in manager.trading_pairs:
    symbol_stats = stats['messages'][symbol]
    print(f"\n{symbol}:")
    print(f"  Message Rate: {symbol_stats['message_rate']:.2f}/sec")
    print(f"  Average Latency: {stats['performance'][symbol]['avg_latency']:.2f}ms")
```

Advanced Usage Examples:
----------------------

1. Custom Market Analysis:
```python
# Combined market analysis
async def analyze_market(manager, symbol):
    snapshot = manager.get_market_snapshot(symbol)
    liquidity = manager.get_liquidity_analysis(symbol)
    indicators = manager.get_technical_indicators(symbol)
    
    return {
        'price': snapshot['price'],
        'volatility': snapshot['volatility'],
        'liquidity_score': liquidity['imbalance_ratio'],
        'rsi': indicators['rsi'],
        'market_depth': {
            'bid_depth': liquidity['bid_liquidity'],
            'ask_depth': liquidity['ask_liquidity']
        }
    }

# Usage
analysis = await analyze_market(manager, 'BTCUSDT')
```

2. Real-time Monitoring:
```python
async def monitor_market_conditions(manager, symbol, threshold=2.0):
    while True:
        ...
        if abs(liquidity['imbalance_ratio']) > threshold:
            print(f"Significant imbalance detected for {symbol}")
            print(f"Imbalance ratio: {liquidity['imbalance_ratio']:.2f}")
            
        await asyncio.sleep(1)  # Check every second

# Usage
asyncio.create_task(monitor_market_conditions(manager, 'BTCUSDT'))
```

Method Details:
-------------

get_market_snapshot(symbol: str) -> Dict[str, Any]:
...
get_relative_strength(base_symbol: str = 'BTC') -> Dict[str, Any]:

    Calculate relative strength metrics compared to base symbol.
    
    Returns:
    - Relative strength ratio
    - Correlation with base asset
    - Cumulative returns
    - Rankings and composite scores
...
------
1. Data Persistence:
   - Historical data is stored in HDF5 format
   - Real-time updates are persisted automatically
   - Data integrity is maintained through atomic operations
...
3. Performance Considerations:
   - Message rates are monitored and throttled if needed
   - Memory usage is managed through periodic cleanup
   - Connection health is actively monitored

4. Best Practices:
   - Always use async/await with WebSocket operations
   - Monitor connection statistics regularly
   - Handle cleanup properly using context managers
